...
<div class=well well-sm><h3><strong>CONFIGURATION OF ELECTIONS</strong></h3></div>	
...
if (isset($_POST['submit'])) {
	$c1=$_REQUEST['name'];
	$qw="select max_votes from catinfo where catname='$c1'"; 
	$qww=mysqli_query($con,$qw);
	$arow=mysqli_fetch_row($qww);
	$csn="select * from $c1.cand_signup";
	$qwww=mysqli_query($con,$qw);
	$arrow=mysqli_num_rows($qwww);
	$mm=($arow[0]-$arrow);
	$can="select * from $c1.voter_signup";
  ...
  if (empty($_POST["roll"])) {
	...
	$c1=$_REQUEST['name'];
			$count=0;
			$rd="select * from $c1.voter_signup where candroll='$roll'";
	...
	if (!preg_match("/^[6-9][0-9]{9}$/",$mobs)) {
	  ...
	  $mobErr1=1;
		...
		$c1=$_REQUEST['name'];
				$count=0;
			$rd="select * from $c1.voter_signup where candmob='$mobs'";
			$res=mysqli_query($con,$rd);
			$count=mysqli_num_rows($res);
			if($count==1)
			{
	...
	else
	{
			...
			$c1=$_REQUEST['name'];
			$mobi=$mob;
			$aw="insert into $c1.voter_signup (candname,candroll,candmob) values('$naam1','$anuk','$mobs')";
